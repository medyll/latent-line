...
<div class="flex items-center justify-between" aria-label="System Footer">
	<div class="flex items-center gap-4">
		...
		<select id="checkpoint-select" tabindex="0" class="rounded border px-2 py-1">
			<option>default</option>
			<option>custom</option>
		</select>
		<!-- Sampler selection -->
		<Label for="sampler-select">Sampler:</Label>
		<select id="sampler-select" tabindex="0" class="rounded border px-2 py-1">
			<option>Euler</option>
			<option>DDIM</option>
		</select>
		<!-- TTS Engine selection -->
		<Label for="tts-select">TTS Engine:</Label>
		<select id="tts-select" tabindex="0" class="rounded border px-2 py-1">
			<option>Coqui</option>
			<option>Google</option>
		</select>
	</div>
	<!-- Export JSON button -->
	<Button onclick={exportModel} aria-label="Export JSON">Export JSON</Button>
</div>
