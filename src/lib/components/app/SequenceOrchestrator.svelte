...
-->
<div class="flex flex-col h-full gap-6 w-full" aria-label="Sequence Orchestrator">
  <!-- Synoptic View (Top) -->
  <Card>
    <CardHeader>
      ...
      <div class="flex flex-row flex-wrap gap-2 mb-4">
        ...
        {:else}
          ...
          {#each timelineStore.slice(0, 10) as event (event.time)}
            <div class="w-32 h-24 border rounded bg-gray-50 flex flex-col items-center justify-center focus:outline focus:ring" aria-label={`Frame ${event.time}`}>
      ...
      <div class="overflow-x-auto flex gap-2">
        ...
        {:else}
          ...
          {#each timelineStore as event (event.time)}
            <div class="w-48 h-16 border rounded bg-white flex flex-col items-center justify-center focus:outline focus:ring" aria-label={`Time ${event.time}`}>
