...
>
    ...
    {#if item.speech}
        <div class="mt-2 flex items-center gap-2 text-sm italic text-gray-700 dark:text-gray-300">
            <MessageCircle class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Speech" />
            <span>"{item.speech}"</span>
        </div>
    {/if}
    {#if item.mood}
        <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
            <Smile class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Mood" />
            <span>Mood: {item.mood}</span>
        </div>
    {/if}
    {#if item.action}
        <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
            <Activity class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Action" />
            <span>Action: {item.action}</span>
        </div>
    {/if}
    {#if item.zoom}
        <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
            <ZoomIn class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Zoom" />
            <span>Zoom: {item.zoom}</span>
        </div>
    {/if}
    {#if item.fx && item.fx.bloom}
        <div class="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
            <Sparkles class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Bloom" />
            <span>Bloom: {item.fx.bloom}</span>
        </div>
    {/if}
    {#if item.audio && item.audio.length > 0}
        <div class="flex items-center gap-2 text-xs mt-1 text-gray-600 dark:text-gray-300">
            <Volume2 class="w-4 h-4 text-gray-400" stroke-width="1.5" aria-label="Audio" />
            <span>Audio:</span>
