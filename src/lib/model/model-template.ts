...
const environmentSchema = z.object({
	...
	ref: z
		.string()
		.optional()
		.refine((v: string | undefined) => !v || isUrlOrFile(v), {
			message: 'ref must be URL or file path'
		})
});
const audioAssetSchema = z.object({
	id: z.string(),
	url: z.string().refine(isUrlOrFile, { message: 'must be URL or file path' }),
...
const resolution = {
	...
	square1024: {
		name: 'square1024',
		description: 'Square 1:1 resolution commonly used for model inputs and previews.',
		parameters: {
			w: 'integer(1024)',
			h: 'integer(1024)',
			aspect_ratio: '1:1',
...
const lighting = {
	...
	dusk: {
		name: 'dusk',
		description: 'Warm, low-angle light with long shadows and reduced contrast — twilight mood.',
		parameters: {
	...
	daylight: {
		name: 'daylight',
		description: 'Neutral, high-energy sunlight with strong directional shadows and high contrast.',
		parameters: {
			intensity: 'decimal(0.0-1.0)',
	...
	tungsten: {
		name: 'tungsten',
		description: 'Warm indoor lighting (incandescent) with low color temperature and cozy mood.',
		parameters: {
			intensity: 'decimal(0.0-1.0)',
	...
	ambient: {
		name: 'ambient',
		description: 'Soft, non-directional fill light that reduces contrast and flattens shadows.',
		parameters: {
			intensity: 'decimal(0.0-1.0)',
...
const speechStyle = {
	...
	shout: {
		// original name: 'shout'
		name: 'shout',
		description: 'Loud, forceful projection with higher energy and clipped cadence.',
		parameters: {
	...
	monotone: {
		...
		name: 'monotone',
		description: 'Flat, even delivery with minimal pitch variation — useful for neutral narration.',
		parameters: {
	...
	playful: {
		...
		name: 'playful',
		description: 'Bouncy, energetic tone with expressive pitch swings and quicker pacing.',
		parameters: {
	...
	formal: {
		...
		name: 'formal',
		description: 'Polished, measured delivery with clear diction and moderate pacing.',
		parameters: {
