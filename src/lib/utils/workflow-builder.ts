...
export function removeNode(workflow: ComfyWorkflow, nodeId: string): void {
	...
	workflow.links = workflow.links.filter((l) => l.fromNode !== nodeId && l.toNode !== nodeId);
	workflow.updatedAt = Date.now();
}
 ...
 */
export function addParameterMapping(workflow: ComfyWorkflow, mapping: ParameterMapping): void {
