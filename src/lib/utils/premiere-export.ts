...
export function generatePremiereXml(model: Model, options: PremiereExportOptions): string {
	...
	const width =
		options.resolution === 'custom'
			? (options.customWidth ?? 1920)
			: getResolutionWidth(options.resolution);
	const height =
		options.resolution === 'custom'
			? (options.customHeight ?? 1080)
			: getResolutionHeight(options.resolution);

	const events = [...model.timeline].sort((a, b) => a.time - b.time);
	const totalDuration =
		events.length > 0
			? events[events.length - 1].time +
				(events[events.length - 1].duration ?? 24) * (1000 / frameRate)
			: 0;
  ...
  <sequence id="${generateSequenceId()}">
	...
	if (includeAudio) {
		const audioEvents = events.filter(
			(e) => e.frame.audio_tracks && e.frame.audio_tracks.length > 0
		);
