...
@media (prefers-color-scheme: dark) {
  :root:not([data-theme='light']) {
    ...
    --border: oklch(0.40 0.01 295);        /* #4e4e62 - Bordure normale */
...
[data-theme='dark'] {
  ...
  --surface2: oklch(0.30 0.01 295);
  --surface3: oklch(0.35 0.01 295);
  --border: oklch(0.40 0.01 295);
...
```html
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Mono:wght@300;400;500&family=DM+Sans:ital,wght@0,300;0,400;0,500;0,700;1,300&display=swap" rel="stylesheet">
```
...
.waveform {
  ...
  background: linear-gradient(to right,
    var(--border) 0%, var(--border) 50%, transparent 50%);
...
.waveform-active {
  background: linear-gradient(to right,
    var(--accent3) 0%, var(--accent3) 50%, transparent 50%);
  background-size: 4px 100%;
}
```
...
```css
.mood-joyful     { --mood-color: var(--accent3); }  /* Teal */
.mood-melancholic { --mood-color: var(--accent2); }  /* Violet */
.mood-anxious    { --mood-color: var(--accent); }   /* Rouge */
.mood-tense      { --mood-color: var(--warning); }   /* Orange */
.mood-serene     { --mood-color: oklch(0.7 0.2 140); }   /* Vert */
```
